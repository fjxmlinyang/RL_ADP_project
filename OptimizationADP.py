...
class OptimizationADPSetUp():
    def __init__(self):
        #model_type is RT_DA, DA, Rolling
        pass
    
    def set_up(self,Input_folder, Output_folder,date, model_type, benchmark):
        self.Input_folder = Input_folder
        self.Output_folder = Output_folder
        self.date = date
        self.model_type = model_type
        self.segments = benchmark
        self.boundary = {}

# class input_parameter(object):
#     def __init__(self):
#         pass
    def change_file(self, filename):
        self.filename = filename

    def input_parameter(self, paranameter_name):
        Data = pd.read_csv(self.filename)
        df  = pd.DataFrame(Data)
        ret = df[paranameter_name]
        self.boundary[paranameter_name] = ret
        return ret

###special E start?
    def input_Estart(self, paranameter_name, LAC_bhour, LAC_last_windows):
        ...
        if LAC_bhour==0 or LAC_last_windows:
            ...
            self.filename = self.Output_folder+'/LAC_Solution_System_SOC_' + str(LAC_bhour-1) + '.csv'
            Data = pd.read_csv(self.filename)
            df = pd.DataFrame(Data)
            Estart = df['SOC'][0]
        return Estart
###special LMP, e_time_periods
    def input_LMP(self, LAC_bhour, LAC_last_windows, probabilistic, paranameter_name):
        if LAC_last_windows:
        # filename = Input_folder + '\LMP_Hindsight' + '.csv'
            self.filename = Input_folder + '/prd_dataframe_wlen_24_'+date + '.csv'
        else:
            # filename = Input_folder+'\LMP_Scenarios_' + 'T' + str(LAC_bhour) +'_DA'+ '.csv'
            if probabilistic:
                self.filename = Input_folder + '/DA_lmp_Scenarios_wlen_' + str(24-LAC_bhour) + '_'+date+'_50' + '.csv'
            else:
                self.filename = Input_folder + '/prd_dataframe_wlen_'+str(24-LAC_bhour)+'_'+date + '.csv'
        ...
        return lmp_quantiles, lmp_scenarios, e_time_periods



#add coefficient
class add_coefficient():
    def __init__(self, name, project_name):
        self.name = name
        self.project_name = project_name
    def add_PSH_0_coeff(self):
        self.


    
    def add_PSH_coeff(self):
...
model_type = 'ADP'
benchmark = 30
method1 = OptimizationADPSetUp()
method1.set_up(Input_folder, Output_folder, date, model_type, benchmark) 

#input PSH
method1.change_file(Input_folder + '/PSH.csv')

PSHmin_g= method1.input_parameter('GenMin')
PSHmax_g = method1.input_parameter('GenMax')
PSHmin_p = method1.input_parameter('PumpMin')
PSHmax_p = method1.input_parameter('PumpMax')
PSHcost = method1.input_parameter('Cost')
PSHefficiency =  list(method1.input_parameter('Efficiency')) 
PSHname =  list(method1.input_parameter('Name')) 

#input Reservoir
method1.change_file(Input_folder + '/Reservoir.csv')
Emin= method1.input_parameter('Min')
Emax= method1.input_parameter('Max')
Ename= method1.input_parameter('Name')
Edayend= float(method1.input_parameter('End'))
...
LAC_bhour = 1
##E start?
Estart = method1.input_Estart('Estart', LAC_bhour, LAC_last_windows)
#read LMP
lmp_quantiles, lmp_scenarios, e_time_periods = method1.input_LMP(LAC_bhour, LAC_last_windows, probabilistic, 'LMP')
...
###########################read the  LMP############################################
    ...
    ####be careful, since it is rolling method, the LMP shall be inoput each time
    if LAC_bhour==0 or LAC_last_windows:
        Estart = df['Start']
    else:
