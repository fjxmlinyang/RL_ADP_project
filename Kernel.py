...
class RL_Kernel():
    ...
    def calculate_optimal_soc(self):
        self.curr_model_para = CurrModelPara(self.LAC_last_windows, self.probabilistic, self.RT_DA, self.date, self.curr_time, self.curr_scenario)
    ...
    def calculate_new_soc(self, initial_soc):
        ...
        pre_model = RLSetUp(psh_system_2, e_system_2, lmp_2, self.pre_curve, ADP_train_model_para, model_1)
        SOC_initial = initial_soc
        pre_model.optimization_model_with_input(SOC_initial)
        return pre_model.optimal_profit
...
#after we get the current self.optimal_profit and self.optimal_soc_sum, we have to update the curve
    ...
    # get previous point profit
        if self.optimal_soc_sum - 1 < self.curve.lo_bd:
            ...
            self.previous_point_profit = self.calculate_new_soc(self.second_point_soc_sum)
        else:
            self.previous_point_soc_sum = self.optimal_soc_sum - 1#self.curve.steps
